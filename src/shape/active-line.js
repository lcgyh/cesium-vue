...
export class EntitiesLineOpation extends ShapeBase {
	constructor(arg) {
		super(arg);
		this.startPointShapeIds = [] // 起始点链接的设备id 集合
		this.endPointShapeIds = [] // 结束点链接的设备id 集合
		this.selectColor = Cesium.Color.DODGERBLUE.withAlpha(1) // 设备选中后的颜色
	...
	shapeAddDraw() {
		...
		const positions =Array.isArray(shapePosition[0])? _this.getPosition(shapePosition):shapePosition
		this.currentEntity = this.viewer.entities.add({
			id: this.shapeId,
			name: this.shapeName,
			polyline: {
				positions: positions,
				material: Cesium.Color.fromCssColorString(_this.shapeColor).withAlpha(1),
				width: 1,
				show: true,
			}
		})
	...
	cancelSelectCurrentEntity() {
		...
		this.currentEntity.polyline.material = Cesium.Color.fromCssColorString(this.shapeColor).withAlpha(1)
    }
	...
	shapeAddPoint() {
		...
		for (var i = 0; i < pointPosition.length; i++) {
			...
			addPointEntitys.push(this.viewer.entities.add({
				name: `编辑点${i}`,
				position: Cesium.Cartesian3.fromDegrees(currentPoint[0], currentPoint[1]),
	...
	shapeDeformationDraw(type, point, pick) {
		...
		if (type === 'MOUSE_MOVE') {
			...
			if (this.editIndexType === '_line_') {
				...
				points = pointsList
				const positions = _this.getPosition(points)
	...
	addLinPont(point) {
		...
		temporaryPoints.push(this.viewer.entities.add({
			name: `临时点0`,
	...
	getPosition(shapePosition) {
		...
		for (var i = 0; i < shapePosition.length; i++) {
			position.push(Cesium.Cartesian3.fromDegrees(shapePosition[i][0], shapePosition[i][1]))
		}
	...
	delAddLinPont() {
		...
		for (var i = 0; i < temporaryPoints.length; i++) {
			this.shapeDeleteDraw(temporaryPoints[i].id)
		}
	...
	removeAddEntitys() {
		...
		for (var i = 0; i < addEntitys.length; i++) {
			this.shapeDeleteDraw(addEntitys[i].id)
		}
	...
	updateSelectCurrentShape() {
		...
		const pointPosition = this.shapePosition.concat()
		const positions = this.getPosition(pointPosition)
		this.currentEntity.polyline.positions = new Cesium.CallbackProperty(() => {
			return positions
		}, false)
